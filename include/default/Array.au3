...
Func _ArrayDisplay($ar_2DArray, $sTitle = "ListView array 1D and 2D Display", $i_ShowOver4000 = 1, $i_Transpose = 0, $GUIDataSeparatorChar = "|", $GUIDataReplace = "~")
	...
	For $c = 0 To UBound($ar_TempSingle) - 1
		...
		EndIf
	Next
	_GUICtrlListView_SetColumnWidth ($searchlistView, 2, $LVSCW_AUTOSIZE)
	If UBound($ar_2DArray, 2) = 1 Then _GUICtrlListView_SetColumnWidth ($searchlistView, 1, $LVSCW_AUTOSIZE)
	GUISetState()
	...
	Do ; LOOP
		...
		Select
			...
			Case $msg[0] = $hndButton_Array1Box_TextSelect
				...
				If _GUICtrlListView_GetItemCount ($searchlistView) Then $a_indices = _GUICtrlListView_GetSelectedIndices ($searchlistView, True)
