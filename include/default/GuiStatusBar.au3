...
Func _GUICtrlStatusBar_Create($hWnd, $vPartEdge = -1, $vPartText = "", $iStyles = -1, $iExStyles = 0x00000000)
	...
	Local $aPartWidth[1], $aPartText[1], $hWndSBar, $x, $iStyle = BitOR($WS_CHILD, $WS_VISIBLE), $iExStyle, $iLast
...
Func _GUICtrlStatusBar_GetBorders($hWnd)
	...
	If _WinAPI_InProcess ($hWnd, $__ghSBLastWnd) Then
		$ret = _SendMessage($hWnd, $SB_GETBORDERS, 0, $borders_struct_pointer, 0, "int", "ptr")
	Else
		$iSize = DllStructGetSize($borders_struct)
		$Memory_pointer = _MemInit ($hWnd, $iSize, $struct_MemMap)
		$ret = _SendMessage($hWnd, $SB_GETBORDERS, 0, $Memory_pointer, 0, "int", "ptr")
...
Func _GUICtrlStatusBar_GetIcon($hWnd, $iIndex = 0)
	If $Debug_SB Then _GUICtrlStatusBar_ValidateClassName($hWnd)
	Return _SendMessage($hWnd, $SB_GETICON, $iIndex, 0, 0, "int", "int", "hwnd")
...
Func _GUICtrlStatusBar_GetParts($hWnd)
	...
	If _WinAPI_InProcess ($hWnd, $__ghSBLastWnd) Then
		$aParts[0] = _SendMessage($hWnd, $SB_GETPARTS, $iCount, $pParts, 0, "int", "ptr")
	Else
		$iParts = DllStructGetSize($tParts)
		$pMemory = _MemInit ($hWnd, $iParts, $tMemMap)
		$aParts[0] = _SendMessage($hWnd, $SB_GETPARTS, $iCount, $pMemory, 0, "int", "ptr")
...
Func _GUICtrlStatusBar_GetRectEx($hWnd, $iPart)
	...
	If _WinAPI_InProcess ($hWnd, $__ghSBLastWnd) Then
		$iResult = _SendMessage($hWnd, $SB_GETRECT, $iPart, $pRect, 0, "int", "ptr")
	Else
		$iRect = DllStructGetSize($tRect)
		$pMemory = _MemInit ($hWnd, $iRect, $tMemMap)
		$iResult = _SendMessage($hWnd, $SB_GETRECT, $iPart, $pMemory, 0, "int", "ptr")
...
Func _GUICtrlStatusBar_GetText($hWnd, $iPart)
	...
	If _WinAPI_InProcess ($hWnd, $__ghSBLastWnd) Then
		_SendMessage($hWnd, $SB_GETTEXT, $iPart, $pBuffer, 0, "int", "ptr")
	Else
		$pMemory = _MemInit ($hWnd, $iBuffer, $tMemMap)
		_SendMessage($hWnd, $SB_GETTEXT, $iPart, $pMemory, 0, "int", "ptr")
...
Func _GUICtrlStatusBar_GetTipText($hWnd, $iPart)
	...
	$pBuffer = DllStructGetPtr($tBuffer)
	If _WinAPI_InProcess ($hWnd, $__ghSBLastWnd) Then
		_SendMessage($hWnd, $SB_GETTIPTEXT, _WinAPI_MakeLong ($iPart, 4096), $pBuffer, 0, "int", "ptr")
	Else
		$pMemory = _MemInit ($hWnd, 4096, $tMemMap)
		_SendMessage($hWnd, $SB_GETTIPTEXT, _WinAPI_MakeLong ($iPart, 4096), $pMemory, 0, "int", "ptr")
...
Func _GUICtrlStatusBar_SetBkColor($hWnd, $iColor)
	...
	If $iColor = $CLR_DEFAULT Then Return '0x' & Hex($CLR_DEFAULT)
	Return $iColor
...
Func _GUICtrlStatusBar_SetIcon($hWnd, $iPart, $hIcon = -1, $sIconFile = "")
	...
	If $hIcon = -1 Then ; Remove Icon
		Return _SendMessage($hWnd, $SB_SETICON, $iPart, $hIcon, 0, "int", "hwnd") <> 0
	ElseIf StringLen($sIconFile) > 0 Then ; set icon from file
		...
		$result = $result[0]
		If $result > 0 Then $result = _SendMessage($hWnd, $SB_SETICON, $iPart, DllStructGetData($tIcon, 1), 0, "int", "ptr")
...
Func _GUICtrlStatusBar_SetParts($hWnd, $iaParts = -1, $iaPartWidth = 25)
	...
	If _WinAPI_InProcess ($hWnd, $__ghSBLastWnd) Then
		_SendMessage($hWnd, $SB_SETPARTS, $iParts, $struct_parts_pointer, 0, "int", "ptr")
	Else
		$iSize = DllStructGetSize($struct_parts)
		$Memory_pointer = _MemInit ($hWnd, $iSize, $struct_MemMap)
		_MemWrite ($struct_MemMap, $struct_parts_pointer)
		_SendMessage($hWnd, $SB_SETPARTS, $iParts, $Memory_pointer, 0, "int", "ptr")
		_MemFree ($struct_MemMap)
	EndIf
	_GUICtrlStatusBar_Resize($hWnd)
	Return 1
...
Func _GUICtrlStatusBar_SetText($hWnd, $sText = "", $iPart = 0, $iUFlag = 0)
	...
	If _WinAPI_InProcess ($hWnd, $__ghSBLastWnd) Then
		$ret = _SendMessage($hWnd, $SB_SETTEXT, BitOR($iPart, $iUFlag), $sBuffer_pointer, 0, "int", "ptr")
	Else
		$Memory_pointer = _MemInit ($hWnd, StringLen($sText) + 1, $struct_MemMap)
		_MemWrite ($struct_MemMap, $sBuffer_pointer)
		$ret = _SendMessage($hWnd, $SB_SETTEXT, BitOR($iPart, $iUFlag), $Memory_pointer, 0, "int", "ptr")
		_MemFree ($struct_MemMap)
	EndIf
	Return $ret <> 0
...
Func _GUICtrlStatusBar_SetTipText($hWnd, $iPart, $sText)
	...
	If _WinAPI_InProcess ($hWnd, $__ghSBLastWnd) Then
		_SendMessage($hWnd, $SB_SETTIPTEXT, $iPart, $sBuffer_pointer, 0, "int", "ptr")
	Else
		$Memory_pointer = _MemInit ($hWnd, StringLen($sText) + 1, $struct_MemMap)
		_MemWrite ($struct_MemMap, $sBuffer_pointer, $Memory_pointer, StringLen($sText) + 1)
		_SendMessage($hWnd, $SB_SETTIPTEXT, $iPart, $Memory_pointer, 0, "int", "ptr")
		_MemFree ($struct_MemMap)
	EndIf
